...
showProduct(itemFive());

function showProduct(juiceData) {
  document.querySelector(".products-showed").innerHTML = "";

  for (let index = 0; index < juiceData.length; index++) {
    const element = juiceData[index];
...
categoryItems.forEach((categoryitem) => {
  categoryitem.addEventListener("click", function (e) {
    ...
    let newData = juiceData.filter((c) => {
...
function getVals() {
  ...
  juiceData = juiceData.filter(function (i) {
    return parseInt(i.price) >= slide1 && parseInt(i.price) <= slide2;
  });
  juiceData = juiceData.sort(function (a, b) {
    return parseFloat(a.price) - parseFloat(b.price);
  });

  showProduct(juiceData);
  addToCart();
}
function showProduct(juiceData) {
  document.querySelector(".products-showed").innerHTML = "";
