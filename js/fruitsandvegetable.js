...
showProduct(itemThree());

function showProduct(frutasData) {
  document.querySelector(".products-showed").innerHTML = "";

  for (let index = 0; index < frutasData.length; index++) {
    const element = frutasData[index];
    ...
    <h1 class="product-title picture">${element.name}</h1>
...
categoryItems.forEach((categoryitem) => {
  categoryitem.addEventListener("click", function (e) {
    ...
    let newData = sabziData.filter((c) => {
