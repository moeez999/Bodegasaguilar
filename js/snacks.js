...
showProduct(itemSeven());

function showProduct(snackData) {
  document.querySelector(".products-showed").innerHTML = "";

  for (let index = 0; index < snackData.length; index++) {
    const element = snackData[index];
...
function getVals() {
  ...
  snackData = snackData.filter(function (i) {
    return parseInt(i.price) >= slide1 && parseInt(i.price) <= slide2;
  });
  snackData = snackData.sort(function (a, b) {
    return parseFloat(a.price) - parseFloat(b.price);
  });

  showProduct(snackData);
  addToCart();
}
function showProduct(snackData) {
  document.querySelector(".products-showed").innerHTML = "";
